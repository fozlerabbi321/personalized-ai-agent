...
from app.agent.prompts import build_nova_system_prompt
...
async def general_response_node(state: AgentState) -> dict:
    """
    Handle general learning queries with a direct Gemini response as Nova.
    Tokens from this node ARE streamed to the client.
    Nova's Socratic persona, dynamic tone rules, and chat history are injected.
    """
    raw_messages = list(state.get("messages", []))
    system_prompt = build_nova_system_prompt(raw_messages)
