...
from app.agent.prompts import build_kairo_system_prompt
...
async def general_response_node(state: AgentState) -> dict:
    """
    Handle general career coaching questions with a direct Gemini response as Kairo.
    Tokens ARE streamed.
    """
    raw_messages = list(state.get("messages", []))
    system_prompt = build_kairo_system_prompt(raw_messages)
